                  ...
                  [ media []
                    [ mediaLeft []
                      [ image (OneByOne X48) []
                        ...
                        [ img [ style "border-radius" "50%", Avatar.src (Profile.avatar profile) ] [] ] 
                      ]
                    , mediaContent []
...
viewTabs before selected after =
    ...
    --tabs { tabsModifiers | style = Boxed}  [][] <|
        List.concat
            [ List.map (viewTab False []) before
            , [ viewTab True [ class "active" ] selected ]
            , List.map (viewTab False []) after
            ]
...
viewPagination toMsg page (Model feed) =
    let
        ...
        prevPage = if page == 1 then 1 else (page - 1)
        nextPage = if page == totalPages then totalPages else (page + 1)  
    ...
    if totalPages > 1 then
       ...
       pagination Right [
         ...
         ,style "aria-label" "pagination"
       
       ]
        [ paginationPrev [onClick (toMsg prevPage)] [ text "Prev" ]
        , paginationNext [onClick (toMsg nextPage)] [ text "Next"     ]
        , paginationList []  
...
pageLink : (Int -> msg) -> Int -> Bool -> PaginationListItem msg
pageLink toMsg targetPage isActive =
    paginationLink isActive [onClick (toMsg targetPage)] [ text (String.fromInt targetPage) ]
    -- paginationLink isActive [] [ text (String.fromInt targetPage) ]
